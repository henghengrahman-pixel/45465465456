...
test('RapidAPI streaming adapter accepts nested response and removes non-football',async t=>{
  const fake=http.createServer((req,res)=>{res.setHeader('content-type','application/json');res.end(JSON.stringify({data:{matches:[
    {match_id:'football-1',match_status:'live',league:{name:'Premier League'},home:{name:'Arsenal',logo:'https://img.test/a.png'},away:{name:'Chelsea',logo:'https://img.test/c.png'},timestamp:1787922000,streams:[{name:'Utama',url:'https://video.test/live.m3u8',type:'hls'}]},
    {match_id:'fight-1',match_status:'live',league_name:'ONE Championship',home_team_name:'Fighter A',away_team_name:'Fighter B',servers:[{url:'https://video.test/fight.m3u8'}]}
  ]}}))});
  ...
  const proc=spawn(process.execPath,['server.js'],{cwd:process.cwd(),env:{...process.env,PORT:String(port),DATA_DIR:dir,JWT_SECRET:'test-secret-value-123456789',ADMIN_PASSWORD:'TestingPassword123',FOOTBALL_API_KEY:'',LIVE_FOOTBALL_API_KEY:'',RAPIDAPI_KEY:'test-key',STREAM_API_HOST:'fake.test',STREAM_API_BASE:`http://127.0.0.1:${fakePort}`,STREAM_MATCHES_PATH:'/matches'}});t.after(()=>proc.kill());
  await new Promise((resolve,reject)=>{const end=Date.now()+8000;(function ping(){fetch(`http://127.0.0.1:${port}/health`).then(r=>r.ok&&resolve()).catch(()=>{});if(Date.now()>end)return reject(Error('server timeout'));setTimeout(ping,100)})()});
  const base=`http://127.0.0.1:${port}`,health=await fetch(base+'/health').then(r=>r.json());assert.equal(health.streamProvider,'rapidapi');
  const streams=await fetch(base+'/api/streams').then(r=>r.json());assert.equal(streams.ok,true);assert.equal(streams.matches.length,1);assert.equal(streams.matches[0].home_team_name,'Arsenal');assert.equal(streams.matches[0].league_name,'Premier League');
